...
>(
    ...
    // Generate Substrait binary artifact for SQL profiles (before consuming merged_selection_map)
    let substrait_artifact = if TypeId::of::<TCompilationProfile>()
        == TypeId::of::<sql_network_protocol::SQLAndJavascriptProfile>()
    {
        Some(generate_substrait_artifact(
            root_entity,
            merged_selection_map.inner_ref(),
        ))
    } else {
        None
    };
    ...
    // Emit Substrait binary artifact if generated above
    if let Some(substrait_result) = substrait_artifact {
        match substrait_result {
            Ok(substrait_bytes) => {
