...
pub trait Database: Sized {
    ...
    fn intern<T: Clone + Hash + DynEq + 'static>(&self, value: T) -> MemoRef<T, Self>;
    fn intern_ref<T: Clone + Hash + DynEq + 'static>(&self, value: &T) -> MemoRef<T, Self>;
...
#[derive(Debug)]
pub(crate) struct InternalStorage<Db: Database + Sized> {
...
pub fn intern<Db: Database, T: Clone + Hash + DynEq + 'static>(
    db: &Db,
    value: T,
) -> MemoRef<T, Db> {
...
fn intern_from_param<Db: Database, T: Clone + DynEq>(db: &Db, param_id: ParamId) -> MemoRef<T, Db> {
