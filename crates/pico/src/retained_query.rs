...
pub fn retain<Db: Database, T>(db: &Db, memo_ref: MemoRef<T, Db>) -> RetainedQuery {
    debug_assert!(std::ptr::eq(db, memo_ref.db));
    match db.get_storage().retained_calls.entry(memo_ref.derived_node_id) {
...
pub fn clear_retain<Db: Database>(db: &Db, mut retained_query: RetainedQuery) {
    match db.get_storage().retained_calls.entry(retained_query.derived_node_id) {
        Entry::Occupied(mut occupied_entry) => {
            (*occupied_entry.get_mut()) -= 1;
