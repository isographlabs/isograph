...
use crate::{
    dependency::{NodeKind, TrackedDependencies}, derived_node::{DerivedNode, DerivedNodeId}, dyn_eq::DynEq, epoch::Epoch, intern::Key, Database, InnerFn
};
...
pub fn execute_memoized_function<Db: Database>(
    ...
    let (time_updated, did_recalculate) =
        if let Some(derived_node) = db.get_storage().internal.get_derived_node(derived_node_id) {
            if db.get_storage().internal.node_verified_in_current_epoch(derived_node_id) {
                (
                    db.get_storage().internal.current_epoch,
                    DidRecalculate::ReusedMemoizedValue,
                )
            } else {
                db.get_storage().internal.verify_derived_node(derived_node_id);
...
fn derived_node_changed_since<Db: Database>(db: &Db, derived_node_id: DerivedNodeId, since: Epoch) -> bool {
    let inner_fn = if let Some(derived_node) = db.get_storage().internal.get_derived_node(derived_node_id) {
        if let Some(rev) = db.get_storage().internal.get_derived_node_revision(derived_node_id) {
