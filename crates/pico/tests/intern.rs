use pico::{Storage, MemoRef, SourceId, Database};
use pico_macros::{memo, Source, Db};
use thiserror::Error;
...
) -> Result<MemoRef<char, TestDatabase>, FirstLetterError> {
    db.get(input_id)
        .value
        .chars()
        .next()
...
fn process_input(
    db: &TestDatabase,
    input_id: SourceId<Input>,
) -> Result<MemoRef<char, TestDatabase>, ProcessInputError> {
