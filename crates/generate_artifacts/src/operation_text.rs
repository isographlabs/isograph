use common_lang_types::{derive_display, QueryOperationName, ServerObjectEntityName};
...
pub(crate) fn generate_operation_text<'a, TNetworkProtocol: NetworkProtocol>(
    ...
    match persisted_documents {
        Some(pd) => {
            ...
            if pd.options.include_extra_info {
                let query_extra_info = TNetworkProtocol::generate_query_extra_info(
                    query_name,
                    operation_name,
                    indentation_level + 1,
                );
