...
impl Database {
    pub fn calculate<TParam, TOutput>(
        ...
        param: TParam,
        inner_fn: impl Fn(&mut Database, TParam) -> TOutput,
    ) -> TOutput
    where
        TParam: Hash + Clone,
        ...
        if self.nodes.contains_key(&node_id) {
            ...
            let maybe_changed = self
                ...
                .filter_map(|dependency| {
                    if dependency.time_verified != self.current_epoch {
            ...
            let _node = DerivedNode {
                value: Box::new(value.clone()),
                time_verified: self.current_epoch,
                time_calculated: 0,
                dependencies: vec![],
                calculate: Box::new(|database| Box::new(inner_fn(database, param.clone()))),
            };
