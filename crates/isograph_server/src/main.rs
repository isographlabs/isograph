...
async fn execute_query(
    ...
    tracing::info!("Received query request: plan_id={}, params={:?}", req.plan_id, req.params);
    ...
    // 4. Execute query
    let df = ctx.execute_logical_plan(logical_plan)
        .await
        .map_err(|e| {
    ...
    tracing::info!("Query executed successfully, returning {} bytes", buffer.len());
    Ok(buffer)
}
