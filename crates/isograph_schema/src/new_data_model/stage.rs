...
use crate::{
    CreateError, LazyValidation, MapWithNonfatalDiagnostics, NestedDataModelSelectable,
    NetworkProtocol,
};
...
pub trait DataModelStage: Copy + Clone + Debug + PartialEq + PartialOrd + Eq + Ord + Hash {
    type Resolution<T, E: CreateError>;
    type Location;

    type Selectables<TNetworkProtocol: NetworkProtocol>;
...
impl DataModelStage for NestedStage {
    ...
    type Selectables<TNetworkProtocol: NetworkProtocol> =
        MapWithNonfatalDiagnostics<SelectableName, NestedDataModelSelectable<TNetworkProtocol>>;
}
