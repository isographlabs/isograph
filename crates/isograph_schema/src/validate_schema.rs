...
use isograph_lang_types::{
    ...
    ServerScalarId, TypeAnnotation, UnvalidatedScalarFieldSelection, UnvalidatedSelection,
    VariableDefinition,
};
use thiserror::Error;

use crate::{
    ClientField, ClientFieldVariant, ClientPointer, FieldType, ImperativelyLoadedFieldVariant,
...
pub type ValidatedClientPointer = ClientPointer<
    <ValidatedSchemaState as SchemaValidationState>::ClientPointerSelectionScalarFieldAssociatedData,
    <ValidatedSchemaState as SchemaValidationState>::ClientPointerSelectionLinkedFieldAssociatedData,
    <ValidatedSchemaState as SchemaValidationState>::VariableDefinitionInnerType,
>;
...
pub type ValidatedFieldDefinitionLocation =
    FieldType<ServerFieldId, ClientFieldId, ClientPointerId>;
...
impl SchemaValidationState for ValidatedSchemaState {
    ...
    type ClientPointerSelectionScalarFieldAssociatedData = ValidatedScalarFieldAssociatedData;
    type ClientPointerSelectionLinkedFieldAssociatedData = ValidatedLinkedFieldAssociatedData;
    ...
    pub fn validate_and_construct(
        ...
        let updated_server_fields = match validate_and_transform_server_fields(fields, &schema_data)
        {
            ...
            Err(new_errors) => {
                ...
                // Because fields flows into updated_client_pointers, we cannot optimistically
                // continue here.
                // TODO: figure out whether this can be worked around.
            }
        };

        let updated_client_pointers = match validate_and_transform_client_pointers(
            client_pointers,
            &schema_data,
        ...
        if errors.is_empty() {
            ...
            Ok(Self {
                client_pointers: updated_client_pointers,
...
fn transform_object_field_ids(unvalidated_object: SchemaObject) -> SchemaObject {
    ...
    let validated_encountered_fields = unvalidated_encountered_fields
        ...
        .map(|(encountered_field_name, value)| match value {
            ...
            FieldType::ClientPointer(client_pointer_id) => (
                encountered_field_name,
                FieldType::ClientPointer(client_pointer_id),
            ),
        })
...
fn validate_and_transform_client_pointers(
    _client_pointers: Vec<UnvalidatedClientPointer>,
    _schema_data: &ServerFieldData,
    _server_fields: &[ValidatedSchemaServerField],
) -> Result<Vec<ValidatedClientPointer>, Vec<WithLocation<ValidateSchemaError>>> {
    Ok(vec![])
}
...
fn validate_and_transform_client_fields(
    ...
    client_pointers: &[ValidatedClientPointer],
...
fn validate_field_type_exists_and_is_scalar(
    ...
    match parent_object.encountered_fields.get(&scalar_field_name) {
        Some(defined_field_type) => match defined_field_type {
            ...
            FieldType::ClientPointer(client_pointer_id) => {
                let client_pointer = &client_pointers[client_pointer_id.as_usize()];

                Err(WithLocation::new(
                    ValidateSelectionsError::FieldSelectedAsScalarButTypeIsNotScalar {
                        field_parent_type_name: parent_object.name,
                        field_name: scalar_field_name,
                        target_type: "a client pointer",
                        target_type_name: schema_data
                            .object(client_pointer.parent_object_id)
                            .name
                            .into(),
                    },
                    scalar_field_selection.name.location,
    ...
    match (parent_object.encountered_fields).get(&linked_field_name) {
        Some(defined_field_type) => match defined_field_type {
            ...
            FieldType::ClientPointer(client_pointer_id) => {
                ...
                let object_id = client_pointer.parent_object_id;
                ...
                let _argument_definitions: Vec<
                    WithSpan<VariableDefinition<SelectableServerFieldId>>,
                > = vec![];
                todo!();
