...
use isograph_lang_types::{
    ...
    UnvalidatedSelection, VariableDefinition, ST_CLASS, ST_DECORATOR, ST_KEYWORD, ST_METHOD,
    ST_NUMBER, ST_OPERATOR, ST_PARAMETER, ST_PROPERTY, ST_STRING, ST_TYPE, ST_VARIABLE,
};
    ...
    let entrypoint_declaration = tokens
        .with_span(|tokens| {
            let parent_type = tokens
                .parse_string_key_type(IsographLangTokenKind::Identifier, ST_CLASS)
                .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
            let dot = tokens
                .parse_token_of_kind(IsographLangTokenKind::Period, ST_OPERATOR)
                .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
            let client_field_name = tokens
                .parse_string_key_type(IsographLangTokenKind::Identifier, ST_METHOD)
                .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
...
fn parse_optional_selection_set(
    ...
    let open_brace: Result<WithSpan<IsographLangTokenKind>, WithSpan<crate::LowLevelParseError>> =
        tokens.parse_token_of_kind(IsographLangTokenKind::OpenBrace, ST_OPERATOR);
    ...
    while tokens
        .parse_token_of_kind(IsographLangTokenKind::CloseBrace, ST_OPERATOR)
        .is_err()
    {
...
fn parse_delimited_list<'a, TResult>(
    ...
    if let Ok(end_span) = tokens.parse_token_of_kind(closing_token, ST_OPERATOR) {
    ...
    loop {
        ...
        if tokens.parse_token_of_kind(delimiter, ST_OPERATOR).is_err() {
            return Err(WithSpan::new(
...
fn parse_comma_line_break_or_curly(tokens: &mut PeekableLexer<'_>) -> ParseResultWithSpan<()> {
    let comma = tokens.parse_token_of_kind(IsographLangTokenKind::Comma, ST_OPERATOR);
    if comma.is_ok()
...
fn parse_optional_alias_and_field_name(
    ...
    let field_name_or_alias = tokens
        .parse_string_key_type::<StringKey>(IsographLangTokenKind::Identifier, ST_PROPERTY)
        .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
    let colon = tokens.parse_token_of_kind(IsographLangTokenKind::Colon, ST_OPERATOR);
    let (field_name, alias) = if colon.is_ok() {
        (
            tokens
                .parse_string_key_type(IsographLangTokenKind::Identifier, ST_PROPERTY)
                .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?,
...
fn parse_directives(
    ...
    while let Ok(token) = tokens.parse_token_of_kind(IsographLangTokenKind::At, ST_DECORATOR) {
        let name = tokens
            .parse_string_key_type(IsographLangTokenKind::Identifier, ST_DECORATOR)
            .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
...
fn parse_argument(
    ...
    let argument = tokens.with_span(|tokens| {
        let name = tokens
            .parse_string_key_type(IsographLangTokenKind::Identifier, ST_PARAMETER)
            .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
        tokens
            .parse_token_of_kind(IsographLangTokenKind::Colon, ST_OPERATOR)
            .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
    ...
    from_control_flow(|| {
        to_control_flow::<_, WithSpan<IsographLiteralParseError>>(|| {
            let _dollar_sign = tokens
                .parse_token_of_kind(IsographLangTokenKind::Dollar, ST_VARIABLE)
                .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
            let name = tokens
                .parse_string_key_type(IsographLangTokenKind::Identifier, ST_VARIABLE)
                .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
        ...
        to_control_flow::<_, WithSpan<IsographLiteralParseError>>(|| {
            let string = tokens
                .parse_source_of_kind(IsographLangTokenKind::StringLiteral, ST_STRING)
                .map(|parsed_str| {
        ...
        to_control_flow::<_, WithSpan<IsographLiteralParseError>>(|| {
            let number = tokens
                .parse_source_of_kind(IsographLangTokenKind::IntegerLiteral, ST_NUMBER)
                .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
        ...
        to_control_flow::<_, WithSpan<IsographLiteralParseError>>(|| {
            let open = tokens
                .parse_token_of_kind(IsographLangTokenKind::OpenBrace, ST_OPERATOR)
                .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
        ...
        to_control_flow::<_, WithSpan<IsographLiteralParseError>>(|| {
            let bool_or_null = tokens
                .parse_source_of_kind(IsographLangTokenKind::Identifier, ST_VARIABLE)
                .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
...
fn parse_variable_definition(
    ...
    let variable_definition = tokens.with_span(|tokens| {
        let _dollar = tokens
            ...
            .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
        let name = tokens
            .parse_string_key_type(IsographLangTokenKind::Identifier, ST_OPERATOR)
            .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?
            .to_with_location(text_source);
        tokens
            .parse_token_of_kind(IsographLangTokenKind::Colon, ST_OPERATOR)
...
fn parse_optional_default_value(
    ...
    if tokens
        .parse_token_of_kind(IsographLangTokenKind::Equals, ST_OPERATOR)
        .is_ok()
    {
...
fn parse_type_annotation(
    ...
    from_control_flow(|| {
        to_control_flow::<_, WithSpan<IsographLiteralParseError>>(|| {
            let type_ = tokens
                .parse_string_key_type(IsographLangTokenKind::Identifier, ST_TYPE)
                .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;

            let is_non_null = tokens
                .parse_token_of_kind(IsographLangTokenKind::Exclamation, ST_TYPE)
                .is_ok();
            if is_non_null {
        ...
        to_control_flow::<_, WithSpan<IsographLiteralParseError>>(|| {
            ...
            tokens
                .parse_token_of_kind(IsographLangTokenKind::OpenBracket, ST_TYPE)
                .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;

            let inner_type_annotation = parse_type_annotation(tokens)?;
            tokens
                .parse_token_of_kind(IsographLangTokenKind::CloseBracket, ST_TYPE)
                .map_err(|with_span| with_span.map(IsographLiteralParseError::from))?;
            let is_non_null = tokens
                .parse_token_of_kind(IsographLangTokenKind::Exclamation, ST_TYPE)
                .is_ok();
