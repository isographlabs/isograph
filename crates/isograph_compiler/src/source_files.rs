...
pub fn read_all_source_files(db: &mut IsographDatabase) -> Result<(), Box<dyn Error>> {
...
pub fn read_updates(db: &mut IsographDatabase, changes: &[SourceFileEvent]) -> Result<(), Box<dyn Error>> {
...
fn read_iso_literals_from_project_root(db: &mut IsographDatabase) -> Result<IsoLiteralMap, Box<dyn Error>> {
