...
pub(crate) fn db_macro(item: TokenStream) -> TokenStream {
    ...
    let output = quote! {
        impl ::pico::Database for #struct_name {
            ...
            fn intern<T: Clone + std::hash::Hash + ::pico::DynEq + 'static>(&self, value: T) -> ::pico::MemoRef<T, Self> {
                ::pico::intern(self, value)
            }

            fn intern_ref<T: Clone + std::hash::Hash + ::pico::DynEq + 'static>(&self, value: &T) -> ::pico::MemoRef<T, Self> {
