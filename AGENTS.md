...
- Within one enum, either every variant's payload carries its span or none does. Do not mix `WithSpan`-wrapped payloads with bare ones; the span belongs on the item wrapper or inside the payload struct.
