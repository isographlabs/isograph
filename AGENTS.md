...
- We do not have anything we do not use. A token kind, field, variant, or recorded value exists only because something reads it. Consume that records a semantic token and extends a span is a reader. An AST field is a reader only if resolve or a later pass uses it. Tests-only is not a reader. `!` after a type is consumed as `GraphQLTypeName` and extends the annotation span; that is use. An `exclamation` field on the annotation would have no reader, so it is not a field.
