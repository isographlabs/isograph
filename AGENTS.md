...
- Whether a type is span-carrying is decided once, at the type: every use of an enum is `WithSpan`-wrapped or none is, and within one enum, every variant's payload carries its span or none does. Do not mix wrapped and bare at either level; the span belongs on the item wrapper or inside the payload struct.
