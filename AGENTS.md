...
- We do not have anything we do not use. A token kind, field, variant, or recorded value exists only because something reads it. Consume that records a semantic token and extends a span is a reader. An AST field is a reader only if resolve or a later pass uses it. Tests-only is not a reader. `!` after a type is consumed to decide Null vs not: no semantic token, not on the annotation span, no hover target of its own. A position on `!` resolves to the parent.
