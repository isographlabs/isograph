...
- Each doc must, at all times that we are actively working on it, conform to several standards:
  ...
  - Every before or after snippet opens with a comment naming the file the code lives in (for example `// crates/isograph_parser/src/matched_brackets.rs`); generated code carries its `// generated by $file` marker instead.
