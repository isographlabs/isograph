...
- Parser interned-key wrappers are named `$RoleWrapper` and wrap the `common_lang_types` interned type for that role, matching isograph (`FieldArgumentNameWrapper(FieldArgumentName)`, `SelectionNameWrapper(SelectionName)`, `ClientFieldNameWrapper(SelectableName)`). They do not implement `From<StringKey>`: `string_key_newtype!` already does that on the inner type. Construction is `token.interned().map(SelectionNameWrapper)`.
- Selection and Selectable are different types. A selection is an item in a selection set. A selectable is a field or pointer on a type. Do not wrap `SelectableName` for a selection, and do not name a selection type `Selectable*`.
