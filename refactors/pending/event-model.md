...
4. filesystem-events.md. `IsographState` is the pico database. `DiskChanged` with `Presence` interns or removes `DiskFile`. Files arrive through `isograph send`.
5. config-includes.md. Config field `includes`: a `Vec` of globs, Biome last-match-wins. `SourceScope`.
