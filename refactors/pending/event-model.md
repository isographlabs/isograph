...
7. extract-iso-literals-from-file.md. After 4. Extract does not parse. pico memo `extract_iso_literals_from_file_content` (the whole vec). pico memo `iso_literal_extraction` (file + index). `IsoLiteralExtraction` with text, context, and byte start. Move `IsographState` to `isograph_compiler`.
