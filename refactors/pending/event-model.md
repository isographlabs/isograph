...
14. lsp-dispatch.md. Later. isograph `LSPRequestDispatch` / `LSPNotificationDispatch` / `LSPRuntimeError` copied into `isograph_lsp`. Session posts `IsographEvent::Lsp` (message plus reply sender) and does not interpret methods after initialize. Request handler is `method_not_found` returning `Vec<IsographEffect>` (one immediate `SendLspResponse`). Notification chain is `isograph/event`. Continue is no effects.
