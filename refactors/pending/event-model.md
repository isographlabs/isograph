...
6. filesystem-watcher.md. Later. After 4 and 5. Two changes in that file: prefix `Absent`, then the OS watcher posts `DiskChanged`. `isograph start --filesystem watch|injected`. Default `Watch`.
...
14. lsp-dispatch.md. Later. isograph request and notification chains in `handle`. Session posts `LspRequest` / `LspNotification` and does not interpret methods after initialize. Request Continue is `method_not_found`. Notification Continue is no effects. `isograph/event` is the first notification arm.
