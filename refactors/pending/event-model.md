...
12. lsp-port.md. The `{slug}.port` TCP listener is LSP. Each TCP connection is one client and one `session`. `isograph/event` params are the `--file` JSON (`HelloWorld` / `Quit` / `DiskChanged`); the session deserializes them and posts that `IsographEvent`. `handle` is unchanged. `isograph send` does the handshake then that notification. Requests other than initialize are `MethodNotFound` on that connection. `Kill` unlinks the port file then `process::exit(0)`.
13. lsp-sessions.md. Later. Per-connection caps and a writer. Stale relative to this slice until rewritten.
14. lsp-tokens.md. Later. Domain requests (`semanticTokens/full`). Stale relative to this slice until rewritten.
