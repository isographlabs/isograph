...
14. lsp-dispatch.md. Later. isograph `LSPRequestDispatch` / `LSPNotificationDispatch` / `LSPRuntimeError` copied into `isograph_lsp`. `IsographEvent` is `Lsp` | `Ingested`. Session posts `Lsp`. `handle` of `Lsp` is isograph's server-loop match. Request chain has zero handlers; Continue is `method_not_found` returning `Vec<IsographEffect>` (one immediate `SendLspResponse`). Notification chain is `isograph/event`; effects are not `SendLspResponse`. `--file` is `Ingested`.
