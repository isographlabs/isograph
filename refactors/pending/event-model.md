...
14. lsp-dispatch.md. Later. isograph `LSPRequestDispatch` / `LSPNotificationDispatch` / `LSPRuntimeError` copied into `isograph_lsp`. `handle` calls them. Session posts `LspRequest` / `LspNotification` and does not interpret methods after initialize. Request Continue is `method_not_found`. Notification Continue is no effects. `isograph/event` is the first notification arm.
