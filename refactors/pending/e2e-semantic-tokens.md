...
Parse is keyed on literal text only. isograph's `memoized_parse_iso_literal` takes `text_source: TextSource` and comments "we should not pass the text source here! Whenever the iso literal moves around the page, we break memoization, due to this parameter." i2 does not pass it. File-absolute tokens still change when the literal moves: isograph `get_semantic_tokens` (issue 548) notes that encoded positions cannot be reused after typing before the literal. That is the concat memo, not parse.
