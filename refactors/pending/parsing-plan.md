...
1. `peek-then-parse.md`. `parse_non_constant_value` peeks without `commit`, then a parse function requires the first token. `parse_variable_name` requires `$` then the identifier. `CursorPeek` is unchanged.
