...
The `From`-constructed parents are resolve-position-parent-conversion.md's addition; every enum here is all-delegate, so no other macro change is needed.
...
`BracketItem`'s arms carry the `parent.into()` of the parent-conversion doc; both conversions are reflexive there, since `RawToken` and `Bracketed` declare `BracketItemParent` themselves. `RawToken` delegates every variant, the bracket arms converting through the parent `From`s and the ordinary arm reflexively; `NonBracketToken` is a leaf, its derived fallback the whole impl:
...
impl ::resolve_position::ResolvePosition for RawToken {
    ...
    fn resolve<'a>(
        ...
        match self {
            RawToken::NonBracket(inner) => inner.resolve(parent.into(), position),
...
2. resolve-position-parent-conversion.md lands.
...
- resolve-position-parent-conversion.md returns to pending with `RawToken`'s bracket arms as its consumer and lands between the two changes here; resolve-option-like-enums.md parks in refactors/past with no consumer, every enum here being all-delegate.
...
2. The parent-conversion doc lands.
