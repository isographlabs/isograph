...
///
/// `resolved_node` is `T::ResolvedNode`. A `for<'a>` equality
/// `T: ResolvePosition<ResolvedNode<'a> = SomeEnum<'a>>` implies `T: 'static`
/// (rustc 1.97) and is not written. Leftover still has to return that same
/// associated type, so `E: 'static` and
/// `for<'a> E: ResolvePosition<ResolvedNode<'a> = T::ResolvedNode<'a>>`.
/// `E: 'static` is that HRTB, not a borrow on leftover.
...
pub struct Slot<T: ResolvePosition, E: ResolvePosition + 'static>
where
    for<'a> E: ResolvePosition<ResolvedNode<'a> = <T as ResolvePosition>::ResolvedNode<'a>>,
    ...
    for<'a> <T as ResolvePosition>::ResolvedNode<'a>: From<
...
impl<T: ResolvePosition, E: ResolvePosition + 'static> ::resolve_position::ResolvePosition for Slot<T, E>
where
    for<'a> E: ResolvePosition<ResolvedNode<'a> = <T as ResolvePosition>::ResolvedNode<'a>>,
    for<'a> <E as ResolvePosition>::Parent<'a>: From<<T as ResolvePosition>::Parent<'a>>,
    for<'a> <T as ResolvePosition>::ResolvedNode<'a>: From<
...
The derive does not write `'static`. It writes `type ResolvedNode<'a> = <T as ResolvePosition>::ResolvedNode<'a> where Self: 'a` and keeps the type's own parameters and where-clause via `split_for_impl`. `E: 'static` is on `Slot`, not on the associated types.
...
struct Slot<T: ResolvePosition, E: ResolvePosition + 'static>
where
    for<'a> E: ResolvePosition<ResolvedNode<'a> = <T as ResolvePosition>::ResolvedNode<'a>>,
    for<'a> <E as ResolvePosition>::Parent<'a>: From<<T as ResolvePosition>::Parent<'a>>,
    for<'a> <T as ResolvePosition>::ResolvedNode<'a>:
