...
`IsographResolutionNode` is unchanged: every variant stays, `OpenBracket` and `CloseBracket` included, since matched groups still have both tokens as leaves. The derive-site changes are the attribute edits shown above (the leaf types' `parent_type` repointing, `ChunkedGroup`'s `Matched` wrappings becoming bare `#[resolve_field]`, `ChunkContentItem` losing its `Unmatched` payloads with the variants); the macro is untouched and every expansion follows shapes the docs already carry.
