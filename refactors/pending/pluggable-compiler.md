...
The extraction seam is `HostLanguage` in extract-iso-literals.md. `extract_iso_literals` finds iso literals in a file and returns `WithErrors` (`item` / `errors`). `IsoLiteralExtraction` carries the common contents and span plus `THostLanguage::LiteralContext`. The first implementor is `TypeScriptHostLanguage`. File extensions for the watcher stay a later field on that trait.
