...
Origin for the types: `crates/isograph_lang_types/src/declarations/selection_declaration.rs` and `client_selectable_declaration.rs`. Delta: each selection sits in a `Slot`; `name` and `reader_alias` are both `SelectionNameWrapper` (isograph uses `SelectableName` and `SelectableAlias`; distinguishing them was not useful); `arguments` is `Option<WithSpan<ArgumentList>>` rather than `Vec<SelectionFieldArgument>`; directives are absent until parse-directives.md; `Selection` is an enum, not `SelectionType<ScalarSelection, ObjectSelection>`.
...
`reader_alias` and `name` are isograph's field names. Both are `SelectionNameWrapper` over `SelectableName`. A selection node is not named `Selectable*`.
...
`require_selection_set` has no caller until parse-fields.md. It takes `#[expect(dead_code)]`. `consume_selection_set` and `parse_selection` are used from tests via `parse_each_chunk`.
...
`SelectionSet` iterates the vec, each hit descending with the container's path. `Selection` delegates. `ObjectSelection` / `ScalarSelection` expand with `parent_variant` wrapping. `SelectionNameWrapper` is an interned-key leaf.
...
1. `Expectation::{SelectionSet, Selection}`, `selections.rs`, `ArgumentListParent::{ScalarSelection, ObjectSelection}`, the Selection slot pin, `UnparsedChunkItemsParent::SelectionSlot`, the resolution-node variants, the tests. `cargo test -p isograph_parser` and the clippy pre-commit hook pass. If `ArgumentListParent` trips a clippy enum-variant-name lint, `#[expect]` it on that enum.
