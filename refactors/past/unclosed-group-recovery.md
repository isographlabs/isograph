...
1. Synthetic closing in the bracket matcher, with unmatched brackets extracted. Grouping returns the tree beside its errors, `(WithSpan<MatchedBrackets>, Vec<BracketError>)`: an unclosed group closes synthetically at its level's end and stays a group (its closing an `Option`), a stray close leaves the tree entirely with its positions falling back to the container, and the tree's type then has no unmatched variants at all, the refinement a type parameter would buy, achieved by construction. Chunking inherits a clean item type, the grammar stage's unmatched rule disappears (nothing exists to stop at), `Taken` collapses into the chunk's own item type and is deleted (`take_next` yields `Option<&WithSpan<ChunkContentItem>>`), the literal above parses as a field declaration with the selection `bar`, and `foo( { bar, baz }` parses as `foo` with a degraded argument interior instead of losing the subtree. This is the `Closing::Synthetic` design the bracket docs weighed before choosing demotion (refactors/past/bracket-matching-cases.md, resilient-parser.md). The costs: `closing` becomes optional everywhere it is read, the crossing-junk cases (`foo { (} )`) need re-deciding, and the landed matcher, chunking, resolution, and their tests all churn. If adopted, this supersedes parsing-standards.md's stop-at-unmatched rule.
