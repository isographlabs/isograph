...
`parse_iso_literal` takes `text: &str`. `parse_chunked_iso_literal` takes `WithSpan<ChunkedRoot>`. A group's interior is the `ChunkedLevel` in `group.children`. A level that is not inside brackets is `ChunkedRoot`: `Vec<WithSpan<ChunkContentItem>>`.
...
`ChunkStream::new` takes a content slice. `parse_stream` spans then leftover. `parse_one_chunk` is `parse_stream` plus trailing-separator fold on `Ok` only, then re-join. `parse_chunked_iso_literal` streams the unpartitioned vec. Diagnostics are not leftover items. Leftover items sit on `Slot.extra`. There are no extra chunks on the tree. `Slot.item` is `Some` when the form parsed. Diagnostics go through `report_error` in `parse_stream` and `parse_bracket_interior_type`; `errors.push` in `parse_iso_literal` (`EmptyLiteral`). `parse_one_chunk` only folds extra. Inner `parse_*` stays `Result`. Artifact generation requires `errors.is_empty()` and that the earlier-stage lists are empty. `require_end` is a method on `ChunkStream`.
...
`IsoLiteralItem` is the declaration enum. `parse_stream` of the unpartitioned vec returns `IsoLiteralParse`. That value is the tree. Empty is `None` from `parse_iso_literal`.
...
`IsoLiteralItem`'s `parent_type` is `IsoLiteralParsePath<'a>`. `EntrypointDeclaration`'s `parent_type` is `IsoLiteralParsePath<'a>`. `SelectionFieldArgument`'s `parent_type` is `SelectionFieldArgumentSlotPath<'a>`. `ObjectEntry`'s `parent_type` is `ObjectEntrySlotPath<'a>`.
...
`ChunkedLevel`'s vec is private to the `chunk` module. `len` is the chunk count. `parse_each_chunk` maps each chunk through `parse_one_chunk`. Empty is the caller's. Enums deleted: `ChunkedLevelParent`, `ChunkParent`, `Singleton`, `ExtraChunks`, `ExtraChunksPath`, `IsoLiteralSlotPath`, `parse_singleton`, `parse_nested_singleton`, `MultipleDeclarations`. `ChunkedRoot` is listed.
...
`parse_stream` leftover and failed-form diagnostics go through `stream.cursor().report_error`. `parse_each_chunk` builds each child stream with `parent.stream_chunk`. `parse_one_chunk` folds the trailing separator into extra on `Ok` only, then re-joins the slot span.
...
Empty is the caller (`parse_iso_literal` pushes `EmptyLiteral` and returns `None`; a `[...]` with zero chunks is `Expected(TypeAnnotation, EndOfChunk)`).

`parse_*` is all-or-nothing. There is no recovered prefix of a selection. `entrypoint Foo.$ asdf` fails at `$`. `item` is `None`. `extra` is the unread remainder, or the original contents when remaining is `None`.

A type that contains a group stores `Vec<WithSpan<Slot<P, UnparsedChunkItems>>>`. Feature docs write those types.
...
- `parse_*`: implements a form made of several items. Parameter is `&mut ItemCursor`. First `Err` is returned. Shared iteration is `parse_each_chunk`, `parse_stream`, or `spanning`. A nested list takes the cursor.
- Diagnostic: `report_error` in `parse_stream` and `parse_bracket_interior_type`; `errors.push` in `parse_iso_literal` (`EmptyLiteral`). `parse_one_chunk` only folds extra. Not stored on the tree.
...
A failed list chunk is `item: None` plus the chunk's items in `Slot.extra`. Leftover after a successful list item is `item: Some` plus leftover items and `report_error(Expected(Separator, ...))`. Tokenless diagnostics (empty literal) go through `errors.push` with no `UnparsedChunkItems`. Leftover after a complete declaration is `Slot.extra` plus `report_error(Expected(EndOfDeclaration, found))` at the first leftover item that is not a line break. `thiserror` formats `ParseError`. Suggestions are produced later from `(expected, found)`.

There is no `errors()` walk on the tree. Tests read the vec `parse_iso_literal` was passed.

## Totality

`parse_iso_literal` returns `None` on an empty chunked literal and a tree otherwise. A position in a parsed region resolves to a grammar leaf. A position in leftover or failed items resolves through `UnparsedChunkItems`. A position on whitespace or a dropped unmatched-bracket region resolves to the nearest containing node. An empty literal has no grammar tree; the diagnostic is `EmptyLiteral`.
...
A tree enum is wrapped in `WithSpan` at its slot. `Slot.item` and `Slot.extra` are `Option<WithSpan<_>>`. Each other struct field that is a node is `WithSpan`. A name is a newtype over an interned key in a `WithSpan`; each role is its own type. The converted scalar is the `i64`. A position on `.`, `$`, `!`, `:`, or `to` resolves to the containing node. There is no keyword-marker type. Resolve walks the optimistic tree only.

`ResolvePosition` is derived. The one blanket delegation is `Box<T>` (parse-variables.md). A parent is a path alias at one parent, an enum at the second. Chunk-stage `IsographResolutionNode` variants resolve inside `UnparsedChunkItems`.
...
- Unpartitioned sequence: `parse_stream` → `Slot<P, UnparsedChunkItems>`
- Type interior: `parse_one_chunk` on chunk 0
...
- Diagnostic: `report_error` in `parse_stream` and `parse_bracket_interior_type`; `errors.push` in `parse_iso_literal` (`EmptyLiteral`)
...
- parse-variables.md: `parse_type_annotation`, `parse_one_chunk` on `[...]` chunk 0, `NonConstantValueParent::VariableDefault`, `Box<T>` delegation in `resolve_position`
...
A feature is reviewed against this doc when it lands. Amendment sites: the `ItemCursor` and `ChunkStream` impls, `parse_stream`, `parse_one_chunk`, and `parse_each_chunk`. This doc stays in `refactors/pending`.
