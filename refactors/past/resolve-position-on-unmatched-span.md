...
`from_path` is the first arm. `struct_name`, and omitted, are the second.
...
`transparent` still emits no unmatched-span arm. `on_unmatched_span` on an enum is a compile error. Any ident other than `from_path` or `struct_name` is a compile error.
...
1. `on_unmatched_span` on `ResolvePositionArgs`, the two emits, omitted equals `struct_name`, unknown-ident error, enum error, the tests. `cargo test -p resolve_position_macros` and `cargo test -p isograph_parser` pass.
