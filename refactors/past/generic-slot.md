...
This doc’s list has the root pin. A later list appends `(<Selection, UnparsedChunkItems>, SelectionSetPath<'a>)`. `resolved_node` stays on the container. One pin may use `on_unmatched_span = struct_name` (the default). Two or more pins require `from_path`: `struct_name` would emit `ResolvedNode::#struct_name` in every impl.
...
// from crates/resolve_position_macros/src/resolve_position_macro.rs
    let output = match args.self_type_generics.reference() {
        ...
        Some(pins) => {
            ...
            require_from_path_with_pins(pins.0.len(), args.on_unmatched_span.reference())?;
...
Origin: `Slot` in `chunk.rs` after change 2. Delta: `on_unmatched_span = from_path`, `SlotPath` → `IsoLiteralSlotPath`, `Slot` → `IsoLiteralSlot`, the `From` impl. One pin still allows `struct_name`; the variant is no longer `Slot`, so unmatched cannot stay `struct_name`.
...
2. Change 2: `self_type_generics` is a list or omitted. One pin may use `struct_name`. Two or more pins require `from_path`. Slot and Singleton convert to one-element lists. Two-pin macro test. `cargo test -p resolve_position_macros` and `cargo test -p isograph_parser` pass.
