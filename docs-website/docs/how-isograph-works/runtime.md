...
- The developer calls `const HomePage = useResult(fragmentReference);` (or calls `<FragmentRenderer fragmentReference={fragmentReference} />`. `FragmentRenderer` is just a wrapper around `useResult`). This will attempt to read the `Query.HomePage` resolver. This will suspend if there isn't enough data in the store to read all of the data required by the `HomePage` resolver, as is the case when the network request is initially in flight.
  ...
  - `useResult` and `FragmentRenderer` take additional parameters and network request options, not documented here.
...
- This will cause React to retry rendering the `FragmentRenderer` (actually, everything underneath the nearest `Suspense` boundary).
